...
const PublicLayout = ({ showScrollToTop, scrollToTop, activeSection, setActiveSection, showGalleryButton }) => {
    ...
    // التعديل الرئيسي: معالجة التمرير إلى القسم المحدد عند الانتقال من صفحة أخرى
    useEffect(() => {
        // إذا كنا في الصفحة الرئيسية وهناك حالة scrollTo
        ...
        // التمرير إلى الأعلى عند تغيير المسار الرئيسي
        if (!location.hash) {
            window.scrollTo({ top: 0, behavior: 'smooth' });
        }
    }, [location.pathname, location.state, setActiveSection]);
