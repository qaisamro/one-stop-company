...
const AdminStatistics = () => {
  ...
  const fetchStats = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:5000/api/statistics?lang=${form.language}`, {
  ...
  const handleAdd = async () => {
    setLoading(true);
    try {
      await axios.post('http://localhost:5000/api/statistics', form, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleDelete = async (id) => {
    setLoading(true);
    try {
      await axios.delete(`http://localhost:5000/api/statistics/${id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
