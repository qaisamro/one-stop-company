...
const AdminHeader = () => {
  ...
  const fetchLinks = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:5000/api/header?lang=${form.language}`, {
  ...
  const handleUpdateLink = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:5000/api/header/${form.id}`,
        { label: form.label, href: form.href, language: form.language, isSection: form.isSection, content: form.isSection ? form.content : null },
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
  ...
  const handleDeleteLink = async (id) => {
    setLoading(true);
    try {
      await axios.delete(`http://localhost:5000/api/header/${id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
