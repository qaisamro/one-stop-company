...
const AdminTeam = () => {
  ...
  const [form, setForm] = useState({ 
    ...
    photoFile: null, 
    language: 'ar',
    socials: [{ url: '', platform: '' }] // Array for dynamic social links
  });
  ...
  const fetchTeam = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`https://one-stop-company-1.onrender.com/api/team?lang=${form.language}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
      setMembers(response.data);
      setError('');
    } catch (err) {
      setError(t('failed_to_load_team') || 'فشل تحميل أعضاء الفريق');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAdd = async () => {
    ...
    // Convert socials array to JSON string
    formData.append('socials', JSON.stringify(form.socials.filter(s => s.url.trim() !== '')));

    try {
      await axios.post('https://one-stop-company-1.onrender.com/api/team', formData, {
        headers: {
      ...
      setError('');
      setForm({ name: '', position: '', photoFile: null, language: form.language, socials: [{ url: '', platform: '' }] }); // Reset all fields
      setPhotoPreview(null);
      fetchTeam();
  ...
  const handleDelete = async (id) => {
    setLoading(true);
    try {
      await axios.delete(`https://one-stop-company-1.onrender.com/api/team/${id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleEdit = (member) => {
    setForm({ 
      ...
      photoFile: null, // Don't pre-fill file input
      language: member.language,
      socials: member.socials && member.socials.length > 0 ? member.socials : [{ url: '', platform: '' }],
      editingId: member.id // Store ID for update operation
    });
    setPhotoPreview(member.photo); // Show current photo
  };
  ...
  const handleUpdate = async () => {
    ...
    if (form.photoFile) {
      formData.append('photo', form.photoFile);
    } else if (photoPreview && !form.photoFile) { // If no new file, but there was an existing photo
        // Extract filename from full URL if it's a full URL
        const filename = photoPreview.split('/').pop();
        formData.append('existingPhoto', filename);
    } else { // If photo was explicitly removed or never existed
        formData.append('photo', ''); // Send empty string to indicate removal
    }
    formData.append('socials', JSON.stringify(form.socials.filter(s => s.url.trim() !== '')));

    try {
      await axios.put(`https://one-stop-company-1.onrender.com/api/team/${form.editingId}`, formData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'multipart/form-data',
        },
      ...
      setError('');
      setForm({ name: '', position: '', photoFile: null, language: form.language, socials: [{ url: '', platform: '' }] });
      setPhotoPreview(null);
      fetchTeam();
      setTimeout(() => setMessage(''), 3000);
    } catch (err) {
...
>
  ...
  <option value="whatsapp">WhatsApp</option>         {/* ✅ تمت إضافة واتساب */}
  <option value="tiktok">TikTok</option>             {/* جديد */}
  <option value="snapchat">Snapchat</option>         {/* جديد */}
  <option value="telegram">Telegram</option>         {/* جديد */}
  <option value="behance">Behance</option>           {/* للمصممين */}
  <option value="dribbble">Dribbble</option>         {/* للمصممين */}
  <option value="x">X (formerly Twitter)</option>    {/* تويتر الجديد */}
</select>
            ...
            {photoPreview && (
              <div className="mt-4 flex flex-col items-center">
                ...
                <img src={photoPreview} alt="Photo Preview" className="w-32 h-32 object-cover rounded-md shadow-lg border border-gray-200" />
                <button
                  type="button"
                  onClick={() => { setPhotoPreview(null); setForm({ ...form, photoFile: null }); }}
            ...
            <button
              ...
              className={`${form.editingId ? 'bg-blue-600 hover:bg-blue-700' : 'bg-green-600 hover:bg-green-700'} text-white py-3 px-6 rounded-lg w-full transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg mt-4`}
              disabled={loading}
            >
              {loading ? (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      {/* Display social media links in admin panel */}
