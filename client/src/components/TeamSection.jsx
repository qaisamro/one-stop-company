...
import { axiosInstance } from '../api/axiosConfig'; // Use axiosInstance for consistency

// Utility component for social media icons (unchanged)
const SocialIcon = ({ platform, url }) => {
    ...
    switch (resolvedPlatform) {
        ...
        case 'github':
            ...
            break;
        case 'website':
            iconPath = "M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm1 16h-2v-2h2v2zm0-4h-2V8h2v4zm-1-9c-3.866 0-7 3.134-7 7s3.134 7 7 7 7-3.134 7-7-3.134-7-7-7z";
            ariaLabel = "Website";
    ...
    return (
        ...
        >
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl relative">
                <h2
                    className={`text-4xl md:text-5xl font-extrabold uppercase tracking-widest text-it-dark-blue dark:text-white text-center mb-4 md:mb-6 opacity-0 translate-y-4 animate-fade-in-up`}
                >
                ...
                {loading && (
                    <div
                        className={`flex items-center justify-center gap-3 text-it-dark-blue dark:text-it-yellow text-xl py-8 ${
                    ...
                    >
                        ...
                        <p>{t('loading_team') || 'جار تحميل أعضاء الفريق...'}</p>
                    </div>
                )}

                {error && (
                    <p
                        className={`text-it-yellow bg-it-dark-blue p-4 rounded-lg mb-6 text-center shadow-md animate-pulse`}
                    >
                        {error}
                    </p>
                )}
                ...
                {team.length === 0 && !error && !loading ? (
                    <p
                        className={`text-it-dark-blue dark:text-gray-400 text-lg py-8 ${textAlignmentClass}`}
                    >
                            ...
                            {getCurrentTeamMembers().map((member, index) => (
                                ...
                                >
                                    {member.photo_url ? (
                                        <div className="relative w-full h-80 overflow-hidden">
                                            <img
                                                ...
                                                onError={(e) => {
                                                    ...
                                                    console.warn(`Failed to load image for ${member.name}: ${member.photo_url}`);
                                                }}
                                            />

                                            <div
                                                className={`
                                                    absolute ${isArabic ? 'left-0 pl-4' : 'right-0 pr-4'} top-1/2 transform -translate-y-1/2
                                                    flex flex-col gap-0.5
                                                    ${
                                                        isTouchDevice
                                                            ? showSocialsForMember === member.id
                                                ...
                                                {member.socials &&
                                                    member.socials.map((social, idx) => (
                                                        <SocialIcon
                                                            ...
                                                            url={social.social_url || social.url}
                                                        />
                                                    ))}
                                                <button
                                                    onClick={() => handleShareClick(member.id)}
                                                    className="w-10 h-10 flex items-center justify-center bg-it-yellow text-it-dark-blue shadow-md hover:bg-it-yellow-dark transition-colors duration-200 mt-0.5 focus:outline-none focus:ring-2 focus:ring-it-yellow-light focus:ring-opacity-75"
                                        ...
                                        <div className="relative w-full h-80 overflow-hidden bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
                                            <svg
