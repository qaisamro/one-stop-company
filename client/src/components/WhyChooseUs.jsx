...
const CombinedChooseUsAndStatistics = () => {
  ...
  useEffect(() => {
    const fetchFeatures = async () => {
      ...
      try {
        const res = await axios.get(`http://localhost:5000/api/features?lang=${i18n.language}`);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      setErrorStats(null);
      try {
        const response = await axios.get(`http://localhost:5000/api/statistics?lang=${i18n.language}`);
