...
const Projects = () => {
    ...
    useEffect(() => {
        const fetchProjectsAndBackground = async () => {
            ...
            try {
                // Fetch projects
                const projectsResponse = await axios.get(`http://localhost:5000/api/projects?lang=${i18n.language}`);
                console.log('Fetched projects:', projectsResponse.data);
                setProjects(projectsResponse.data.filter(project => project.id));

                // Fetch background image
                try {
                    const backgroundResponse = await axios.get('http://localhost:5000/api/projects/background');
                    setBackgroundImage(backgroundResponse.data.imagePath ? `http://localhost:5000${backgroundResponse.data.imagePath}` : '');
                } catch (bgErr) {
