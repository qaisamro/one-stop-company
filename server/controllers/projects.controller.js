// C:\Users\Lenovo\one-stop-company\server\controllers\projects.controller.js
...
    Project.getAll(lang, (err, results) => { // results هنا ستكون results.rows من النموذج
        if (err) {
...
    const { id } = req.params;
    // في هذه الدالة، لا يوجد 'lang' في req.query، لذا النموذج سيجلب اللغة الافتراضية (العربية حسب التعديل)
    // إذا كنت تريد أن يكون هذا متعدد اللغات، يجب تمرير 'lang' هنا أيضًا
...
        image,
        additional_images, // ستُمرر كمصفوفة وسيتم التعامل معها كـ JSONB في النموذج
        url: url || null,
        language,
        sections: parsedSections // ستُمرر كمصفوفة وسيتم التعامل معها كـ JSONB في النموذج
    };

    Project.create(projectData, (err, result) => { // استلام result من النموذج
        if (err) {
            console.error('Error in createProject:', err);
            return res.status(500).json({ error: 'فشل إضافة المشروع', details: err.message });
        }
        res.status(201).json({ message: 'تمت إضافة المشروع بنجاح', id: result.id }); // استخدام result.id
    });
};
...
    if (!title || !description || !language) { // اللغة مطلوبة للتحديث متعدد اللغات
        return res.status(400).json({ error: 'العنوان والوصف واللغة مطلوبة' });
    }
...
            language, // تأكد من تمرير اللغة هنا
